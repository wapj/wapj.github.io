...
프로그램을 작성하다보면 테스트 데이터가 필요한 경우가 많이 있습니다. 특히나 다른팀과 협을 해야하는데, 같은 테이블을 사용하는 경우에는 테이블에 뭐라도 들어가 있어야 테스트를 해볼 수가 있는데, 제가 데이터를 만들어 주는 것을 기다리는 팀이 있는 상황에는 뭐라도 일단 수동으로 넣고 테스트 해보시라고 말씀을 드리곤 합니다.

그렇지만, 만들어야 하는 데이터가 열개 스무개가 아니라 몇백개가 된다면, 그것만으로도 일이 되겠지요. 이런 상황에서 프로그램을 사용하여 데이터를 만들수 있다면 좋지 않을까? 하는 생각에서 파이썬으로 프로그램을 작성하여 보았습니다. 같이 보실까요.
...
입력해야 하는 데이터가 무려 7개나 있네요.
10개만 해도 타이핑 해야하는 양이 상당합니다.
이런 데이터를 랜덤하게 넣을 수 있는 프로그램이 있으면 좋겠다는 생각이 마구마구 들죠. 우선 랜덤한 데이터를 사용하기 위해 필요한 것들을 찾아봅시다.

## random 모듈을 사용한 랜덤 데이터 만들기

랜덤한 데이터를 만들기 위해 파이썬에서는 `random` 이라는 모듈을 제공하고 있습니다. 그래서 오늘 만들어볼 프로그램에서는 대부분 이녀석으로 처리할 예정입니다.

`random`의 함수를 모두 설명하면 그걸로 오늘 분량이 끝날것이라 오늘은 딱 두가지 함수만 소개 드리겠습니다.
...
> randrange 함수의 숫자 범위는 반폐구간인데요. 무슨말이냐면 위의 예시에서 2010은 포함하고 2021은 포함하지 않는다는 뜻입니다. 학교에서 수학에서는 [2010, 2021) 요렇게 표현합니다. 파이썬에서 시퀀스의 슬라이드 문법도 동일하게 반폐구간이죠.
...
커맨드 라인에서 `mysql -u <유저> -p < student_score.sql` 를 해주면 데이터를 생성할 수 있습니다.

그런데, 이렇게 커맨드를 일일이 실행해주는 것도 귀찮을 수 있습니다. 해당 작업도 할 수 있도록 수정해 봅시다.
...
할일은 덤프파일 경로를 가져오는 것과 패스워드를 입력받도록 하는 것입니다.
...
테스트 데이터를 만들기 위해서 직접 DB연동을 한다음 생성하는 방법도 있을 것입니다.

예제를 만드는중 faker를 더 적극적으로 사용하고, python-fire 를 사용해서 아주 힙한 녀석을 만들어 볼까도 생각하였습니다만 아무래도 기본에 충실한게 좋은 것 같아서 외부라이브러리는 최대한 적게 사용하였습니다.

파이썬은 기본라이브러리도 너무 충실하게 되어 있는 점이 아주 매력적인 언어로 생각됩니다.

최종적인 코드는 [gist](https://gist.github.com/wapj/256605c28d5650620ffbb1f395dd59fe)에 올려두었으니 혹시나 테스트 데이터를 만드실때 참고하시면 될 것 같습니다.
